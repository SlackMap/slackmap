...
var _extendShipit = _interopRequireDefault(
  require('shipit-deploy/lib/extendShipit'),
);
...
const updateTask = (shipit, options) => {
  ...
  _shipitUtils.default.registerTask(
    shipit,
    name + ':update',
    /*#__PURE__*/
    _asyncToGenerator(function*() {
        ...
        _copyPreviousRelease = _asyncToGenerator(function*() {
          ...
          yield shipit.remote(
            _util.default.format(
              'cp %s %s/. %s',
              copyParameter,
              _posix.default.join(shipit.releasesPath, shipit.previousRelease),
              shipit.releasePath,
            ),
          );
        });
      ...
      function _createReleasePath() {
        _createReleasePath = _asyncToGenerator(function*() {
          ...
          shipit.releaseDirname = _moment.default
            .utc()
            .format('YYYYMMDDHHmmss');
          shipit.releasePath = _posix.default.join(
            shipit.releasesPath,
      ...
      function _remoteCopy() {
        _remoteCopy = _asyncToGenerator(function*() {
          const options = _lodash.default.get(
            shipit.config,
            'deploy.remoteCopy',
          ) || {
            rsync: '--del',
          ...
          const uploadDirPath = _posix.default.resolve(
            rsyncFrom,
            shipit.config.dirToCopy || '',
          );

          shipit.log('Copy project to remote servers.');
          yield shipit.copyToRemote(
            `${uploadDirPath}/`,
            shipit.releasePath,
            options,
          );
      ...
      function _setCurrentRevision() {
        _setCurrentRevision = _asyncToGenerator(function*() {
          ...
          const response = yield shipit.local(
          ...
          yield shipit.remote(
            `echo "${shipit.currentRevision}" > ${_posix.default.join(
              shipit.releasePath,
              'REVISION',
            )}`,
          );
